...
const stringEntries = generatedValues.reduce((prevVal, val) => (typeof val === "string" ? prevVal + 1 : prevVal), 0);
const numberEntries = generatedValues.reduce((prevVal, val) => (typeof val === "number" ? prevVal + 1 : prevVal), 0);
document.write(`
    <div>${generatedValues}</div>
    <div>Кількість рядків: ${stringEntries}</div>
    <div>Кількість чисел: ${numberEntries}</div>
`);
if (stringEntries > numberEntries)
    document.write("<div>Рядків більше</div>");
else if (stringEntries < numberEntries)
    document.write("<div>Чисел більше</div>");
else
    document.write("<div>Чисел і рядків порівно</div>");
