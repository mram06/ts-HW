...
document.write(
  ` <div>Задача 1. У localStorage зберігається об’єкт у форматі JSON з ключем «data».</div>
...
const field2: string | number = data.field2;

if (typeof field2 === "number") {
  if (field2 % 2 === 0) document.write(`<div>${field2}, Число парне</div>`);
  else document.write(`<div>${field2}, Число не парне</div>`);
