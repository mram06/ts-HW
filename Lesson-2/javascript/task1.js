"use strict";
document.write(` <div>Задача 1. Вводиться перший номер  місяця якоїсь пори року (3,6,9,12). Визначити пору року. </div>
    ...
    <br>`);
function getSeasonByFirstMonthNumber(monthNumber) {
    switch (monthNumber) {
        case 3:
            return "Весна";
        case 6:
        ...
        case 12:
            return "Зима";
        default:
            const exhaustiveCheck = monthNumber;
            if (monthNumber < 1 || monthNumber > 12)
                throw new Error(`Некоректний місяць (1-12): ${monthNumber}`);
            throw new Error(`Невідомий номер першого місяця пори року: ${monthNumber}`);
    }
}
